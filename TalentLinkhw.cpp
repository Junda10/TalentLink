...
void User::setUserID(int id) {
        // Assuming you have a member variable for the user's ID
        userID = id;
    }
...
void User::bookEvent(queue<User>& bookedEvents, queue<User>& workQueue, int enteredId) {
    int eventIdToBook;
    ...
    while (!workQueue.empty()) {
        ...
        if (currentEvent.id == eventIdToBook) {
            ...
            if (!alreadyBooked) {
                // Set the user ID for the booked event
                selectedEvent.setUserID(enteredId);
...
void User::accountSummary(queue<User>& bookedEvents) const {
    ...
    while (!tempQueue.empty()) {
        ...
        // Assuming event is an instance of the Event class and userId is the User ID
        cout << left << setw(5) << "\nID" << '\t' << setw(20) << "Event Name" << '\t' << setw(20) << "Event Description"
             << '\t' << setw(6) << "Year" << '\t' << setw(6) << "Month" << '\t' << setw(6) << "Day" << '\t' << setw(12) << "Start Time"
             << '\t' << setw(12) << "End Time" << '\t' << setw(8) << "Salary" << '\t' << setw(8) << "User ID\n";

        // Assuming event is an instance of the Event class and userId is the User ID
        cout << left << setw(5) << event.id << '\t' << setw(20) << event.eventName << '\t' << setw(20) << event.eventDescription
             << '\t' << setw(6) << event.year << '\t' << setw(6) << event.month << '\t' << setw(6) << event.day << '\t'
             << setw(12) << event.startTime << '\t' << setw(12) << event.endTime << '\t' << setw(8) << event.salary
             << '\t' << setw(8) << event.getUserID() << "\n";
    }
}
...
int main() {
    ...
    cout << "========================================================================================================================" << std::endl;
            ...
            case 9:{
                eventManager.displayAllEvents();
                   if (bookedEvents.empty()) {
                    ...
                    system("pause");
                    system("cls");
                    break;                cin.ignore(); // Wait for user to press Enter
            system("pause");
            system("cls");
                break;
            }
        ...
        cout<<"========================================================================================================================"<<endl;
            cout << "\n\t 1. Book Event"<< endl;
            cout << "\n\t 2. Show Available Events\t"<< endl;
            cout << "\n\t 3. Booked Event Activity\t"<< endl;
            cout << "\n\t 4. Exit\t"<< endl;
            ...
            switch (userChoice) {
                case 1: {
                    ...
                    do {
                        User currentUserInstance(0, "", "", 0, 0, 0, "", "", 0.0, 0);
                        currentUserInstance.bookEvent(bookedEvents, workQueue, enteredId);
                ...
                if (anotherBooking == 'y' || anotherBooking == 'Y') {
                    // Continue the loop for another booking
                }
                else if (anotherBooking == 'n' || anotherBooking == 'N') {
